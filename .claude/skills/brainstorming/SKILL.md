...
8. **影響ドキュメントを確認** — 設計によって変更が必要な既存ドキュメントを洗い出す（以下参照）
9. **ユーザーが仕様を確認** — 進む前に仕様ファイルをレビューするよう依頼
10. **実装へ移行** — `writing-plans` スキルを呼び出して実装計画を作成
...
**影響ドキュメントの確認:**
設計が確定したら、実装前に影響を受ける既存ドキュメントを洗い出す。これを `writing-plans` に引き継ぐ。

| 設計の影響 | 確認すべきドキュメント |
|------------|----------------------|
| API 追加・変更 | `docs/design/api.md`、`CLAUDE.md` の API 一覧 |
| DB スキーマ変更 | `docs/design/database.md` |
| 新しいアーキテクチャパターン | `.claude/rules/architecture.md` |
| 実装上の制約・慣例 | `.claude/rules/implementation_rules.md` |
| 新しいスクリーン・UX フロー | `docs/design/screens.md` |

影響があるドキュメントをリストアップし、設計ドキュメントに「更新予定のドキュメント」セクションとして記載する。実装完了後に `finishing-a-development-branch` で実際の更新を行う。
